...
def real_data_processing(data_dict, name_dict, n, h, k):
    for name in data_dict.keys():
        ...
        for j in range(i_max): # j=0,...,i_max-1
            ...
            supp = mutual_information_1_entropies_sum(data_array[j*h: (j*h + n)], k=k)
            if supp >= 0:
                mi_array[j] = supp
            else:
                mi_array[j] = 0
        ...
        # save reuslts
        directory_path = f'data/real_data/n_{n}/h_{h}/mi_sum/k_{k}/{name_dict}'
...
if __name__ == "__main__":
    ...
    setup_logger()
    logging.getLogger('core.mutual_information_1_entropies_sum').setLevel(logging.WARNING)
