...
def real_data_binning(data_dict, name_dict, n, h, k):
    for name in data_dict.keys():
        ...
        for j in range(i_max): # j=0,...,i_max-1
            ...
            mi_array[j] = mutual_information_binning_adaptive(data_array[j*h: (j*h + n)], num_bins=k)[0]


        # save reuslts
        directory_path = f'data/real_data/n_{n}/h_{h}/mi_binning/k_{k}/{name_dict}'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--k", type=int, default=1, help="Value for parameter k_bins (default: 1)")
    
    args = parser.parse_args()
    n = args.n
    h = n if args.overlap == 'no' else n // 2
    k = args.k
    logging.info(f"\nParameters: n={n}, h={h}, k_bins(nominal)={k}")
        
    real_data_binning(raw_data['A'], 'A', n, h, k)
    real_data_binning(raw_data['C'], 'C', n, h, k)
