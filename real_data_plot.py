...
from real_data_processing import get_user_parameters, ensure_directory_exists
...
def import_all_data(n, h, k):
    # A files
    file_path1 = f'data/real_data/n_{n}/h_{h}/k_{k}/A/mi_A14F3-TE10034.txt'
    data1 = import_data(file_path1)

    file_path2 = f'data/real_data/n_{n}/h_{h}/k_{k}/A/mi_A14F4-TE10034.txt'
    data2 = import_data(file_path2)

    file_path3 = f'data/real_data/n_{n}/h_{h}/k_{k}/A/mi_A15F3-TE10034.txt'
    data3 = import_data(file_path3)

    file_path4 = f'data/real_data/n_{n}/h_{h}/k_{k}/A/mi_A15F4-TE10034.txt'
    data4 = import_data(file_path4)

    file_path5 = f'data/real_data/n_{n}/h_{h}/k_{k}/A/mi_A16F3-TE10034.txt'
    data5 = import_data(file_path5)

    # C files
    file_path6 = f'data/real_data/n_{n}/h_{h}/k_{k}/C/mi_C11F3-VE.txt'
    data6 = import_data(file_path6)

    file_path7 = f'data/real_data/n_{n}/h_{h}/k_{k}/C/mi_C11F4-VE.txt'
    data7 = import_data(file_path7)

    file_path8 = f'data/real_data/n_{n}/h_{h}/k_{k}/C/mi_C12F3-VE.txt'
    data8 = import_data(file_path8)

    file_path9 = f'data/real_data/n_{n}/h_{h}/k_{k}/C/mi_C13F3-VE.txt'
    data9 = import_data(file_path9)

    file_path10 = f'data/real_data/n_{n}/h_{h}/k_{k}/C/mi_C13F4-VE.txt'
...
def two_sample_ks(mi_A, mi_C, n, k, h, label1, label2):
    alpha = 0.05

    # Two-sample Kolmogorov-Smirnov test
    test_statistics, p_value = ks_2samp(mi_A, mi_C)
    file_path = f'data/real_data/plot/n_{n}/h_{h}/k_{k}/ks_results.txt'
...
def hist_2labels(n, h, k, data1, data2, label1, label2, directory_path):
    fontsize = 12
    title = 1
    figsize = (8, 6)
...
def same_mouse_diff_phase(n, h, k, data):
    ...
    directory_path = f'data/real_data/plot/n_{n}/h_{h}/k_{k}/same_mouse_diff_phase'
    ...
    two_sample_ks(data[label1], data[label2], n, k, h, label1, label2)
    hist_2labels(n, h, k, data[label1], data[label2], label1, label2, directory_path)
...
def diff_mouses_same_class(n, h, k, data):
    file_path = f'data/real_data/plot/n_{n}/h_{h}/k_{k}/ks_results.txt'
    # Scriviamo i risultati nel file di log
    with open(file_path, "a") as f:
        f.write(f"\nDifferent mouses but same class (A/C):")

    directory_path = f'data/real_data/plot/n_{n}/h_{h}/k_{k}/diff_mouses_same_class'
    ensure_directory_exists(directory_path)
    ...
    hist_3labels(n, h, k, data[label1], data[label2], data[label3], label1, label2, label3, directory_path)
...
def diff_mouses_diff_classes(n, h, k, data):
    file_path = f'data/real_data/plot/n_{n}/h_{h}/k_{k}/ks_results.txt'
    with open(file_path, "a") as f:
        f.write(f"\nDifferent mouses from different classes:")

    directory_path = f'data/real_data/plot/n_{n}/h_{h}/k_{k}/diff_mouses_diff_classes'
    ensure_directory_exists(directory_path)
...
def plot_heatmap(matrix, labels, n, h, k):
    ...
    ax.set_title(f"n_{n}/h_{h}/k_{k} Median Absolute Differences")
    plt.savefig(f'data/real_data/plot/n_{n}/h_{h}/k_{k}/heatmap.pdf')

if __name__ == "__main__":
    print("Welcome!\nThis is a script to generate plots for real data analysis.")
    ...
    while True:
        ...
        while check_par == 0:
            ...
            check_par = ensure_directory_exists2(f'data/real_data/n_{n}/h_{h}/k_{k}')

        data = import_all_data(n, h, k)
        # create a directory to save plots we are going to generate
        ensure_directory_exists(f'data/real_data/plot/n_{n}/h_{h}/k_{k}')
        # clean ks_results.txt
        clear_file(f'data/real_data/plot/n_{n}/h_{h}/k_{k}/ks_results.txt')
        # same mouse but different phase
        same_mouse_diff_phase(n, h, k, data)
        # different mouses but same class (A/C)
        diff_mouses_same_class(n, h, k, data)
        # different mouses from different classes
        diff_mouses_diff_classes(n, h, k, data)
        # Compute matrix and plot heatmap
        matrix, labels = compute_median_matrix(data)
        plot_heatmap(matrix, labels, n, h, k)

        print(f'\nPlots for n={n}, h={h}, k={k} have been created!')
