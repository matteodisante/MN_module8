...
def plot_figure_4(files, distribution_name, mi_estimate, theoretical_mi, log_transformed, selected_params, figure_choice):
    ...
    for N in sorted_N_values:
        ...
        if "binning" in mi_estimate.lower():
            ...
            if figure_choice=="4" or "independent" in distribution_name:
                ...
                plt.errorbar(x_values/N, means/theoretical_mi, yerr=sigmas/np.abs(theoretical_mi), xerr=x_errors, linestyle='--', fmt='.', capsize=1, markersize=3, linewidth=0.93, alpha=0.7, label=f'N={N}')
        else:
            if figure_choice=="4" or "independent" in distribution_name:
                plt.errorbar(first_column/N, means, yerr=sigmas, linestyle='--', fmt='.', capsize=1, markersize=3, linewidth=0.93, alpha=0.7, label=f'N={N}')
            elif figure_choice=="4c":
                plt.errorbar(first_column/N, means/theoretical_mi, yerr=sigmas/np.abs(theoretical_mi), linestyle='--', fmt='.', capsize=1, markersize=3, linewidth=0.93, alpha=0.7, label=f'N={N}')
        legend_labels.append(f'N={N}')

    format_plot(mi_estimate=mi_estimate, figure_choice=figure_choice ,distribution_name=distribution_name, selected_params=selected_params, theoretical_mi=theoretical_mi)



def process_figure_4(files, distribution_name, mi_estimate, log_transformed, figure_choice):
    ...
    # Make the plot
    plot_figure_4(final_filtered_files, distribution_name, mi_estimate, theoretical_mi, log_transformed, selected_params, figure_choice)
    ...
    for distribution_name in distribution_choices:
        ...
        for param_value in sorted(distribution_data[distribution_name].keys(), key=lambda x: float(x)):
            ...
            if theoretical_mi is not None:
                ...
                if "binning" in mi_estimate.lower():
                    x_errors = [x_values - xlolims, xuplims - x_values]
                    if is_independent:
...
def main():
    ...
    # Choose the figure
    figure_choice = get_user_choices(["4", "4b", "4c"], "Choose the figure to reproduce:", multiple=False)
    ...
    if figure_choice in ["4", "4c"]:
        process_figure_4(matching_files, distribution_choice, mi_choice, log_transformed, figure_choice)
    if figure_choice == "4b":
